...
ADMIN_CHAT_ID = os.getenv("ADMIN_CHAT_ID", "")  # for alerts
...
# -------------------- CACHE SETUP --------------------
# Use Redis if available, else in-memory TTL cache
try:
    if REDIS_URL:
        import redis
...
def get_market_breadth():
    """Fetch Nifty advance/decline, indices data."""
    # Using yfinance for indices
    indices = {
        "NIFTY 50": "^NSEI",
        "BANK NIFTY": "^NSEBANK",
        "NIFTY IT": "^CNXIT"
    ...
    # For advance/decline, we can use nsetools or scrape (simplified)
    # Here we'll return placeholder
    advance_decline = {"advances": 1200, "declines": 800, "unchanged": 100}
    
    return data, advance_decline
...
def calculate_stock_score(symbol: str) -> dict:
    """
    Compute a score out of 10 for a given stock using multiple factors.
    Returns dict with score, rating, and reasoning.
    """
    try:
        # Fetch data
        ...
        # Score components (each out of 10, weighted)
        tech_score = 0
        # Trend
        ...
        # Sentiment (using AI or rule-based)
        # For now, neutral
        sentiment_score = 5
...
def suggest_portfolio(risk_profile="moderate"):
    """
    Generate a diversified portfolio suggestion with AI scores.
    Returns list of stocks with scores and allocation.
    """
    # Predefined watchlist (could be dynamic based on screening)
    candidates = ["RELIANCE", "TCS", "HDFCBANK", "INFY", "ICICIBANK", "ITC", "SBIN", "BHARTIARTL"]
    scored = []
    for sym in candidates:
        score_data = calculate_stock_score(sym)
        if score_data and score_data["score"] >= 6:  # only good ones
            scored.append((sym, score_data))
    
    # Sort by score
    scored.sort(key=lambda x: x[1]["score"], reverse=True)
    
    # Allocate based on score (higher score = higher weight)
...
def get_ai_analysis(symbol: str) -> str:
    ...
    # Perform analysis (existing stock_ai_advisory function)
...
def start_cmd(m):
    ...
    kb.add("🔍 Stock Analysis", "📊 Market Breadth")
    kb.add("💼 Portfolio Suggestion", "📰 Market News")
    bot.send_message(
        m.chat.id,
        "🤖 <b>AI Stock Advisor Pro</b>\n\n"
        "• Stock Analysis: detailed tech+fundamental+AI\n"
        "• Market Breadth: Nifty indices, A/D ratio\n"
        "• Portfolio Suggestion: AI-scored (9+/10) allocation\n"
...
if __name__ == "__main__":
    ...
    if WEBHOOK_URL:
        ...
        while True:
            time.sleep(60)
            # Optional: send heartbeat to admin
            if ADMIN_CHAT_ID:
                try:
        ...
        # Fallback to polling
        logger.info("No webhook URL, starting polling...")
