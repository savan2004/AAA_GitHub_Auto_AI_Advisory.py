...
def cmd_chart(message):
    ...
    def _run(cid=cid):
        ...
        safe_send(cid, f"📈 Generating chart for <b>{company_name}</b> ({sym})… (~20s)")
        ...
        if success and png_path:
            try:
                ...
                safe_send(cid, build_adv(sym))
        else:
            logger.warning(f"Chart failed for {sym}: {meta_text}")
            safe_send(cid, "⚠️ Chart unavailable, sending text analysis:")
            safe_send(cid, build_adv(sym))
    executor.submit(_run)
...
def chart_button(message):
    ...
    def _run(cid=cid):
        ...
        def _ping():
            _t.sleep(12)
            try:
                safe_send(cid, "⏳ Still scanning… almost done.")
            except Exception:
                pass
        import threading as _th
        _th.Thread(target=_ping, daemon=True).start()
        gen = get_chart_generator()
        gen.send_to_telegram(bot, cid)
    executor.submit(_run)
...
def cmd_portfolio_cmd(message):
    ...
    def _run(cid=message.chat.id):
        safe_send(cid, build_portfolio_card(cid))
    executor.submit(_run)
...
def ai_topic_button(message):
    ...
    def _run(u=uid, tp=topic_prompt):   # Fix 6: default args prevent closure capture bug
        try:
...
def scan_button(message):
    ...
    def _run(cid=message.chat.id, p=profile):
        safe_send(cid, build_scan(p))
    executor.submit(_run)
...
def swing_button(message):
    ...
    def _run(cid=cid, m=mode):
        try:
            ...
            safe_send(cid, f"❌ Swing scan failed: {e}")
    executor.submit(_run)
...
def handle_text(message):
    ...
    if _state.get(uid) == "ai":
        ...
        def _ai(u=uid, t=text):   # Fix 3+7: default args prevent closure bug + exception handling
            try:
                resp = ai_chat_respond(u, t)
                safe_send(u, resp or "⚠️ AI unavailable. Try again in a moment.", reply_markup=ai_keyboard())
            except Exception as _e:
                logger.error(f"AI handler crash uid={u}: {_e}", exc_info=True)
                safe_send(u,
                    "⚠️ <b>AI error.</b> Please try again.\nUse /status to check providers.",
                    reply_markup=ai_keyboard())
        executor.submit(_ai)
        return
            ...
            try:
                ...
                if ticker:
                    ...
                    if 2 <= len(sym_clean) <= 15:
                        ...
                        safe_send(u, f"❌ Could not find <b>{q}</b> on NSE/BSE.\n"
                        "Try: <code>RELIANCE</code>  <code>TCS</code>  <code>HDFC Bank</code>")
            except Exception as _e:
                logger.error(f"_adv error: {_e}")
                safe_send(u, "⚠️ Error. Try again.")
...
def _startup_warmup():
    ...
    for sym in ["RELIANCE", "TCS", "HDFCBANK", "INFY"]:
        try:
            get_hist(sym, "6mo")   # Fix 5: use data_engine (curl_cffi session, no 403)
            logger.info(f"[warmup] {sym} ✅")
        except Exception as _e:
            logger.debug(f"[warmup] {sym}: {_e}")
...
if __name__ == "__main__":
    ...
    # ── FIX 1 CRITICAL: Register webhook with Telegram ─────────────────────
    # Without this, Telegram never sends updates to the server.
    # Must delete old webhook first to avoid 409 Conflict.
    if WEBHOOK_URL:
        try:
            ...
            logger.warning(f"delete_webhook: {_we}")
        try:
            wh_url = f"{WEBHOOK_URL}{WEBHOOK_PATH}"
            bot.set_webhook(url=wh_url, drop_pending_updates=True)
            logger.info(f"✅ Webhook set: {wh_url}")
        except Exception as _se:
            logger.error(f"set_webhook failed: {_se}")
            raise SystemExit(1)
    else:
        logger.warning("WEBHOOK_URL not set — running without webhook (polling mode only)")
