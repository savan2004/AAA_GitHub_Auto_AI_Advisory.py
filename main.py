"""
main.py — AI Stock Advisory Bot (Final Stable Version)
Features: Debug Logging, User-Agent Fix, Robust Screeners.
"""
...
def call_ai(messages, max_tokens=400, system=""):
    errs = []
...
def calc_rsi(c):
    ...
    d = c.diff()
    return round(float((100 - 100/(1 + d.clip(lower=0).rolling(14).mean() / (-d.clip(upper=0)).rolling(14).mean()))).iloc[-1]), 1)
...
AI_T = {"📊 Nifty": "Nifty 50 technical analysis?", "💎 Picks": "3 strong fundamental stocks?"}
...
def to_ai(m): _state[m.chat.id]="ai"; bot.send_message(m.chat.id, "Ask AI:", reply_markup=kb_ai())

@bot.message_handler(func=lambda m: m.text in AI_T)
def ai_btn(m):
    bot.send_message(m.chat.id, "Thinking...")
    r,e = call_ai([{"role":"user","content":AI_T[m.text]}])
...
def handle_all(m):
    ...
    if _state.get(uid) == "ai":
        r,e = call_ai([{"role":"user","content":txt}])
